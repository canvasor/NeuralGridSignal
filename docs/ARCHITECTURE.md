...
- 拉取 OKX 主行情数据。
- 拉取 Binance 辅助行情数据。
- 计算网格适配评分、风险评分、历史 2 天模拟网格表现。
- 可选调用 OpenAI 兼容模型做结构化复核。
- 输出 nofx 可导入策略 JSON。
- 通过 Telegram 发送简洁行动通知。
...
- 回测边界按当前价和 ATR 倍数生成，保证 `atr_multiplier` 会真实影响触发次数和库存偏移。
...
- Telegram 通知。
- 通知包含入选币种、评分、趋势状态、网格层数、ATR 倍数、分配方式、止损和生成文件路径。
...
2. 获取 OKX tickers，按成交额和 OI 过滤低流动性币种。
3. 对候选币获取最近 2 天 15m K 线、最近 7 天 1h K 线、funding、OI、盘口。
...
- 不生成可执行策略，或生成 `total_investment` 很低、`grid_count` 较少、ATR 更宽的观察策略。
- 用于评分勉强合格但风险标签较多时。
